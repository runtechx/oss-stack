## Principais painéis de hospedagem

| Software                |  Código  | Multi-tenant | Multi-servidor | Mail | DNS | AlmaLinux 10 |
| ----------------------- | :------: | :----------: | :------------: | :--: | :-: | :----------: |
| InterWorx               |  Fechado |       ✔      |        ✔       |   ✔  |  ✔  |       AL 9.x      |
| ISPConfig               |  Aberto  |       ✔      |        ✔       |   ✔  |  ✔  |       ✔      |
| ispmanager              |  Fechado |       ✔      |        ✔       |   ✔  |  ✔  |       ✔      |
| VestaCP - refazer       |  Aberto  |       ✖      |        ✖       |   ✔  |  ✔  |       ✖      |
| Control Web Panel (CWP) | Fechado* |       ✔      |        ✖       |   ✔  |  ✔  |       AL 9.X      |
| KeyHelp                 |  Fechado |       ✔      |        ✖       |   ✔  |  ✔  |       ✖       |
| CyberPanel              |  Aberto  |       ✔      |        ✖       |   ✔  |  ✔  |       AL 9.X      |
| DirectAdmin             |  Fechado |       ✔      |        ✔       |   ✔  |  ✔  |       ✔      |
| HestiaCP                |  Aberto  |       ✖      |        ✖       |   ✔  |  ✔  |       ✖      |
| ApisCP                  |  Aberto  |       ✔      |        ✔       |   ✔  |  ✔  |    AL 8.x     |
| Froxlor                 |  Aberto  |       ✔      |        ✖       |   ✔  |  ✔  |       ✖      |
| Plesk                   |  Fechado |       ✔      |        ✔       |   ✔  |  ✔  |       ✔      |
| Sentora                 |  Aberto  |       ✖      |        ✖       |   ✔  |  ✔  |       ✖      |
| aaPanel                 | Fechado* |       ✖      |        ✖       |   ✔  |  ✔  |       AL 9.X      |

Os campos referem-se a:

* Código = Aberto (Open Source) ou Fechado (Proprietário)
* Multi-tenant = Revendedores, clientes e isolamento de contas
* Multi-servidor = Gestão nativa de vários servidores
* Mail = Gestão integrada de email
* DNS = Gestão integrada de DNS
* AlmaLinux 10 = Suporte oficial ou funcional à data atual



## Cobrança de Clientes
* WHMCS
* Blesta
* WISECP
* [BoxBilling](https://hostbillapp.com/feature/proxmox-overview/)

* Opensource
* integra-se com 


| Software          |         Código         | Multi-tenant | Multi-servidor | Mail | DNS |
| ----------------- | :--------------------: | :----------: | :------------: | :--: | :-: |



---




| Blesta            |         Fechado        |       ✖      |        ✖       |   ✖  |  ✖  |           ✔          |
| WHMCS             |         Fechado        |       ✖      |        ✖       |   ✖  |  ✖  |           ✔          |
| HostBill          |         Fechado        |       ✖      |        ✖       |   ✖  |  ✖  |           ✔          |
| WISECP            |         Fechado        |       ✖      |        ✖       |   ✖  |  ✖  |           ✔          |
| BoxBilling        |         Aberto         |       ✖      |        ✖       |   ✖  |  ✖  |           ✔          |
